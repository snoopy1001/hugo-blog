...
本文主要记录个人的 LeeCode 数据库练习过程，内容通常包含以下几个部分：题目简述、解题思路、逻辑步骤、重要知识点、个人犯的错误和正确的 SQL 代码。练习由简单到中等再到困难，每天进步一点点。**最后向评论区分享宝贵经验的广大网友们致谢！**
...
下面是必不可少的总结环节， 我的 SQL 练习先告一段落，通过这一段时间的练习基本熟悉了 SQL 的常用语句，NICE ！JOIN 语句和窗口函数在 SQL 中的重要性不言而喻 ，后面我将继续学习 SQL 中的窗口函数和  JOIN 语句，Let's go !😀